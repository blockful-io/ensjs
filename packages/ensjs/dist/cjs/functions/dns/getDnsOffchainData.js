...
const getAddressRecord_1 = require("../public/getAddressRecord");
const checkValidEnsTxtRecord = async (client, record) => {
    if (record.type !== misc_1.DnsRecordType.TXT)
        return null;
    ...
    const resolverAddress = await (0, getAddressRecord_1.default)(client, {
...
const getDnsOffchainData = async (client, { name, endpoint, strict }) => {
    ...
    try {
        const response = await (0, getDnsTxtRecords_1.getDnsTxtRecords)({ name, endpoint });
        ...
        if (!rrsigRecord)
            throw new dns_1.DnsDnssecWildcardExpansionError();
        ...
        if (invalidRecord)
            throw new dns_1.DnsInvalidTxtRecordError({ record: invalidRecord.recordData });
        throw new dns_1.DnsNoTxtRecordError();
    }
    catch (error) {
        if (!strict)
